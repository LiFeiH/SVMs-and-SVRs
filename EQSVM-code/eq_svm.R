...
eq_svm <- function(X, y, C = 1, kernel = c("linear", "rbf", "poly"),
  ...
  eq_svm_primal_solver <- function(KernelX, y, C = 1, update_deltak,
                                   lambda = 1, tau = 0.5,  m = 0, 
                                   eps = 1e-5, eps.cccp = 1e-2, max.steps = 80, cccp.steps = 10, 
  ...
  if (solver == "dual") {
    ...
    solver.res <- eq_svm_primal_solver(KernelX, y, C, update_deltak, lambda, tau, m,
                                     eps, eps.cccp, max.steps, cccp.steps, batch_size, optimizer,...)
  }
