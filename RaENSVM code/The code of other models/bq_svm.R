...
bq_svm <- function(X, y, C = 1, kernel = c("linear", "rbf", "poly"),
                   ...
                   tau = 1, lambda = 1,
                   eps = 1e-5, eps.cccp = 1e-2, max.steps = 200, cccp.steps = 10,
  ...
  bq_svm_dual_solver <- function(KernelX, y, C = 1, update_deltak,
    ...
    n <- nrow(KernelX)
    H <- manysvms:::calculate_svm_H(KernelX, y)
    e <- matrix(1, n, 1)
    if (tau == 0) {
      u0 <- matrix(0, n, 1)
    } else {
    ...
    for (i in 1:cccp.steps) {
      ...
      u0[u0 > ub] <- ub[u0 > ub]
      u0[u0 < lb] <- lb[u0 < lb]
      u <- manysvms:::clip_dcd_optimizer(H, e, lb, ub, eps, max.steps, u0)$x
  ...
  kso <- manysvms:::kernel_select_option(X, kernel, solver, randx, gamma, degree, coef0)
  KernelX <- kso$KernelX
  X <- kso$X
